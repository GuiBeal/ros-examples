...
MyPublisherNode::MyPublisherNode()
    : Node("my_publisher")
{
  ...
  pCallbackParams_ = this->add_post_set_parameters_callback(
      std::bind(&MyPublisherNode::cbParameters, this, _1));

  pPublisher_ = create_publisher<std_msgs::msg::Int64>(topicName_, rclcpp::SystemDefaultsQoS());

  pTimerPublisher_ = create_wall_timer(std::chrono::duration<double>(period_),
                                       std::bind(&MyPublisherNode::cbPublish, this));
