    ...
    : Node("my_service_server")
{
  ...
  pCallbackParams_ = this->add_post_set_parameters_callback(
      std::bind(&MyServerNode::cbParameters, this, _1));
...
void MyServerNode::cbService(const my_robot_interfaces::srv::ValidateFiscalCode::Request::SharedPtr pRequest,
                             const my_robot_interfaces::srv::ValidateFiscalCode::Response::SharedPtr pResponse)
{
  ...
  if (std::regex_match(pRequest->code, std::regex("\\d{11}?")))
  {
  ...
  else
  {
    // check for ###.###.###-## format
    std::smatch matches;
    if (!std::regex_search(pRequest->code, matches, std::regex("(\\d{3})\\.(\\d{3})\\.(\\d{3})-(\\d{2})")))
    {
  ...
  std::array<int, 9> code;
  for (size_t i = 0; i < 9; ++i)
  {
    code[i] = strCode[i] - '0';
  }
  const int verifier = strtod(strVerifier.c_str(), nullptr);
  ...
  tmp %= 11;
  const int firstVerifier = tmp < 2 ? 0 : 11 - tmp;
  ...
  tmp %= 11;
  const int secondVerifier = tmp < 2 ? 0 : 11 - tmp;
