...
MyClientNode::MyClientNode()
    : Node("my_service_client")
{
...
bool MyClientNode::validateFiscalCode(const std::string &code)
{
  ...
  auto pRequest =
      std::make_shared<my_robot_interfaces::srv::ValidateFiscalCode::Request>();
  pRequest->code = code;

  auto future = pClient_->async_send_request(pRequest); // could also received a callback

  const auto futureStatus = rclcpp::spin_until_future_complete(get_node_base_interface(), future, 10s);
...
int main(int argc, char **argv)
{
  ...
  for (const auto &code : {"610.420.070-20", "123.456.789-10"})
  {
    ...
    RCLCPP_INFO(pNode->get_logger(), "Fiscal code %s is %s.",
